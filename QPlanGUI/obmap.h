...
namespace ob {
    ...
    enum MapType { SearchMap, AggregationMap ,GroupMap};
...
public:
    ...
    void addRange(std::vector<double> vpx, std::vector<double> vpy, std::vector<double> vqx,std::vector<double> vqy);

    bool setOb(double const &ob_x, double const &ob_y);

private:

    bgeo::DRing _outRange;
	//初始化搜索地图的边
