...
namespace pl {
    ...
    void Aplan::setOb(const double &ob_x, const double &ob_y)
    {
        bgeo::DPoint obPnt;
        qDebug()<<" the ob_x is "<< ob_x;
        qDebug()<<" the ob_y is "<< ob_y;
        obPnt.x(ob_x);
        obPnt.y(ob_y);
        ...
        for (auto &it : grid)
        {
          if(it.second.type == bgeo::vertType::ObVert)
          {
              i++;
          }
        }
        qDebug()<<" the ob num is "<<i;
        auto centreObIndex =this->m_map.pnt2Index(obPnt,ob::MapType::AggregationMap);
        ...
        for (auto i = centreObIndex.first - 4; i <= (centreObIndex.first + 4); i++)
        {
            for (auto j = centreObIndex.second - 4; j <= (centreObIndex.second + 4); j++)
            {
                ...
                {
                    std::cout<< " the type is "<<grid[ob::gridIndex(i,j)].type<<std::endl;
                    grid[ob::gridIndex(i,j)].type = bgeo::vertType::ObVert;
//                    std::cout<< "i = "<<i << "   j = "<< j <<std::endl;
                    std::cout<< " the change type is "<<grid[ob::gridIndex(i,j)].type<<std::endl;

                }
            }
        }

        auto Ggrid = this->m_map.GTgrid;
        ...
        qDebug()<<" the groupMode ob num is "<<j;
        ...
        for (auto i = centreObIndex.first - 4; i <= (centreObIndex.first + 4); i++)
        {
            for (auto j = centreObIndex.second - 4; j <= (centreObIndex.second + 4); j++)
            {
                ...
                {
                    Ggrid[ob::gridIndex(i,j)].type = bgeo::vertType::ObVert;
//                    std::cout<< "i = "<<i << "   j = "<< j <<std::endl;
                }
            }
        }
	...
	bool Aplan::AstarPlan()
	{
		...
		if ((target2Grid()) && (start2Grid()))
		{
			...
			qDebug() << "spnt.x = " << this->m_startPnt.x() << "	spnt.y = " << this->m_startPnt.y();
			qDebug() << "sgrid.x = " << this->m_map.ATgrid[m_Sindex].pnt.x() << " sgrid.x = " << this->m_map.ATgrid[m_Sindex].pnt.y();

			qDebug() << "tpnt.x = " << this->m_targetPnt.x() << "	tpnt.y = " << this->m_targetPnt.y();
			qDebug() << "tgrid.x = " << this->m_map.ATgrid[m_Tindex].pnt.x() << " tgrid.x = " << this->m_map.ATgrid[m_Tindex].pnt.y();
