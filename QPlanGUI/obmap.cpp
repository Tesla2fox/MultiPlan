...
void Obmap::drawgrid(const int & SorA)
{
    ...
    for (auto &it : grid)
    {
        if (true)
        {
//            if (it.first.first<220 || it.first.first>260)
//                continue;

//            if(it.first.second < 90 || it.first.second>120)
//                continue;

//            if(it.first.first == 231 && it.first.second ==94)
//            {
//                qDebug()<<"wtf";
//            }
...
bool Obmap::Map2grid()
{
    ...
    for (size_t i = 0; i < m_MaxCol; i++)
    {
        ...
        for (size_t j = 0; j < m_MaxRow; j++)
        {
            ...
            auto _RangeWithIn =  bg::within(pntUnit.pnt,this->_outRange);
            if(!_RangeWithIn)
            {
...
bool Obmap::MapIntersection(const DSegment &sg)
{
    for(auto &it :this->vSegment)
    {
...
gridIndex Obmap::pnt2Index(bgeo::DPoint const & pnt, int const &type)
{
    ...
    if (pnt.y()<this->mWsPoint1.y())
    {
        return rindex;
    }
    if (pnt.x()<this->mWsPoint1.x())
    {
        return rindex;
    }
    auto bais_x = pnt.x() - this->mWsPoint1.x();
    auto bais_y = pnt.y() - this->mWsPoint1.y();
    double step;
...
void Obmap::addRange(std::vector<double> vpx, std::vector<double> vpy, std::vector<double> vqx, std::vector<double> vqy)
{
    this->_outRange.clear();
    bgeo::DPoint pnt;
    pnt.x(vpx.front());
    pnt.y(vpy.front());

    _outRange.push_back(pnt);
    for (size_t i = 0; i < vqx.size(); i++)
    {
        bgeo::DPoint ptq(vqx.at(i), vqy.at(i));
        _outRange.push_back(ptq);
    }
    _outRange.push_back(pnt);
