...
bool Splan::loadMap(ob::Obmap & map)
{
    ...
    else
    {
        ...
        this->apl.setMaxSearchTimes(2000);
        return true;
    }
}
//
...
void Splan::Plan()
{
    ...
    this->TGraph2LocalGraph();

    finish  = std::clock();

    auto duration = (double)(finish - start) / CLOCKS_PER_SEC;

    qDebug()<<"the duration of the Tgraph to the local graph " <<duration;



    //clear the last data of the before search planning
    clearBeforeData();
    ...
    auto initvd = this->findGreedInitVert();
    finish = std::clock();

    duration = (double)(finish - start) / CLOCKS_PER_SEC;

    qDebug()<<" the duration of the find init vert "<<duration;
    if (initvd < 0)
    {
        qDebug()<<"failed ";
        //the aplan failded to find the init vertex
        this->failIndex = -7;
        return ;
    }
    ...
    //this->m_Stree.added_vertex(StreeInitVert);
    boost::add_vertex(StreeInitVert,this->m_Stree);
    start = std::clock();
    //this->GreedPlan(initvd);
    //this->spanTreePlan(initvd);


    this->recursionPlan(initvd);
    //use the method of the recursion to get  a tree of the search place
    finish = std::clock();

    // make the path vector to the spanning tree
    this->vPath2STree();

    // make the spanning tree to a coverage path
    //     this->tree2Path();

    this->STree2Path();
    duration = (double)(finish - start) / CLOCKS_PER_SEC;

    qDebug()<<" the duration of the  coverage  motion planning "<<duration;
...
void Splan::GreedPlan(const int &initvd)
{
    ...
    do {
        ...
        do
        {
            ...
            std::vector<std::pair<VertexDescriptor, size_t>> vNverts = getNeighbour(centreVertDes);
...
double Splan::calVertDis(const STreeVertexDescriptor &index)
{
    double res;
...
void Splan::findSTreePath(const STreeVertexDescriptor &index)
{
    if(this->m_Stree[index].leaf)
    {
...
std::vector<STreeVertexDescriptor> Splan::findChildern(const STreeVertexDescriptor &index)
{
...
int Splan::findGreedInitVert()
{
    ...
    for (auto vit = _b2e_vi.first ; vit != _b2e_vi.second ; vit++)
    {
        ...
        sortDesDis.push_back(std::pair<bgeo::VertexDescriptor,double>(vd,vp.pnt.y()));
        //vvdx.push_back(vp.pnt.x());
        //vvdy.push_back(vp.pnt.y());
    ...
    std::function<bool(DESDIS,DESDIS)> compfunc = [=](DESDIS a,DESDIS b) {if(a.second<b.second){return true;}else{ return false;}
    };
...
std::vector<std::pair<bgeo::VertexDescriptor, size_t> > Splan::getNeighbour(const VertexDescriptor &vd)
{
...
std::vector<bgeo::VertexDescriptor> Splan::getNeighbourNoDir(const VertexDescriptor &vd)
{
